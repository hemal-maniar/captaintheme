...
W_DIR = '../_blog/{0}'.format(date.today().year)
...
def writeCtfFrontMatter():
    ...
    createDirIfNotExists('../assets/Blogs/{0}/'.format(ctfName.replace(" ", "-")))
...
def main():
    global ctfName
    ctfName = input('Enter Blog Name: ')
    writeCtfFrontMatter()
    while True:
        challName = input('Enter Blog name: ')
        descUrl = input('Enter Blog description URL: ')
